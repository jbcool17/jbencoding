...
# MOV to wav - mapping	FFMPEG	ffmpeg -i POKEMONSHORT_PLUSFEATURE_17_v3.mov -vn -acodec pcm_s24le -map 0:0 left.wav && ffmpeg -i POKEMONSHORT_PLUSFEATURE_17_v3.mov -vn -acodec pcm_s24le -map 0:1 right.wav && ffmpeg -i POKEMONSHORT_PLUSFEATURE_17_v3.mov -vn -acodec pcm_s24le -map 0:2 center.wav && ffmpeg -i POKEMONSHORT_PLUSFEATURE_17_v3.mov -vn -acodec pcm_s24le -map 0:3 lfe.wav && ffmpeg -i POKEMONSHORT_PLUSFEATURE_17_v3.mov -vn -acodec pcm_s24le -map 0:4 ls.wav && ffmpeg -i POKEMONSHORT_PLUSFEATURE_17_v3.mov -vn -acodec pcm_s24le -map 0:5 rs.wav
# MOV to wav - mapping	FFMPEG	ffmpeg -i $MOV -vn -acodec pcm_s24le -map 0:1 left.wav && ffmpeg -i $MOV -vn -acodec pcm_s24le -map 0:2 right.wav && ffmpeg -i $MOV -vn -acodec pcm_s24le -map 0:3 center.wav && ffmpeg -i $MOV -vn -acodec pcm_s24le -map 0:4 lfe.wav && ffmpeg -i $MOV -vn -acodec pcm_s24le -map 0:5 ls.wav && ffmpeg -i $MOV -vn -acodec pcm_s24le -map 0:6 rs.wav

# Pull 5.1 from MOV to WAV	FFMPEG	ffmpeg -i {INPUT FILE}.mov -filter_complex "channelsplit=channel_layout=5.1[FL][FR][FC][LFE][BL][BR]" -map "[FL]" LEFT.wav -map "[FR]" RIGHT.wav -map "[FC]" CENTER.wav -map "[LFE]" Lfe.wav -map "[BL]" LEFT_SURROUND.wav -map "[BR]" RIGHT_SURROUND.wav
# 2 Mono Channels to STEREO	FFMPEG	ffmpeg -i left.mp3 -i right.mp3 -filter_complex "[0:a][1:a]amerge=inputs=2[aout]" -map "[aout]" output.wav
